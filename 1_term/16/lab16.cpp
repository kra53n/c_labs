...
errno_t scanStudentArrayFromBinFile(char filename[], Student arr[], int& size)
{
	FILE* f;
	if (fopen_s(&f, filename, "rb")) return 1;

	fread(&size, sizeof(int), 1, f);
	fread(arr, sizeof(Student), size, f);
...
errno_t printStudentArrayToBinFile(char filename[], Student arr[], int size)
{
	FILE* f;
	if (fopen_s(&f, filename, "wb")) return 1;

	fwrite(&size, sizeof(size), 1, f);
	fwrite(arr, sizeof(Student), size, f);
...
int main()
{
	...
	case 2:
		...
		break;
	case 3:
		scanStudentArrayFromBinFile(filenameBin, arr, arrSize);
		break;
	}
	...
	case 2:
		...
		break;
	case 3:
		printStudentArrayToBinFile(filenameBin, arr, arrSize);
		break;
	}
